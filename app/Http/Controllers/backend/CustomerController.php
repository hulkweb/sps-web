...
class CustomerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $users = User::select('*')->where('type','user')->latest();
            if(isset($request->sale_id)){
                $users->where('createBy',$request->sale_id);
            }
            return DataTables::of($users)
                ...
                ->addColumn('actions', function($user) {
                    return '
                        <a href="'.route('customer.edit', $user->id).'" class="btn btn-sm btn-warning">Edit</a>
                ...
                ->rawColumns(['order','status','image', 'actions'])
                ->make(true);
        }
    ...
    public function create()
    {
        ...
        public function edit(string $id)
        {
            ...
            return view('backend.customer.add', compact('user'));
        }
